...
class MusicPlayerScreen{
      MusicPlayerScreen();
      Screen({required String wave,required String link,required double width}){
    //ProjectCreateScreenBloc().startListen();
      return SingleAudioPlayer(wave:audioWave,link: audioLink,width: width);
  }
...
class _SingleAudioPlayerState extends State<SingleAudioPlayer> {
  ...
  Widget build(BuildContext context) {
    ...
    showImage(){
      ...
      double wholeHeight = 90;
      List<Widget> allColumns = [];
     // int itemsToMap =  waveform.length;
      int itemsToMap =  70;
      int samplingFactor = (waveform.length/70).toInt();
          ...
          builder: (c, snapshot) {
            if(snapshot.hasData){
              ...
              for(int i = 0 ; i <reSampledList.length ; i ++){
                ...
                allColumns.add(Expanded(child: Container(height: (reSampledList[i].abs()/maxVal)*wholeHeight,color: Colors.grey,)));
                else
    ...
    showVirtualisation(){
      ...
      return StreamBuilder<Duration>(
          ...
          builder: (c, snapshot) {
            if(snapshot.hasData){
              int pos = 1*((snapshot.data!.inMilliseconds/durationMillis)*reSampledList.length).toInt();
               Text((reSampledList[pos].abs()/maxVal).toString());

              return LinearProgressIndicator(value: reSampledList[pos].abs()/maxVal,);
    ...
    showImageSecondary(){
      ...
      return  Container(height: 90,width: MediaQuery.of(context).size.width,
        child: AudioWaveformWidget(waveColor: Colors.grey,
          waveform: waveform,
          start: Duration.zero,
          duration: waveform.duration,
        ),
    ...
    return WillPopScope (
      ...
      child:SafeArea(child: Scaffold(appBar: AppBar(centerTitle: true,title: Text("Now Playing"),),body:Stack(
        children: [
            ...
            children: [
              Container(margin: EdgeInsets.all(10),
                ...
                child: ClipRRect(
                  ...
                  child:  Image.asset("assets/listen_music.jpeg",fit: BoxFit.cover,),
                ),
              ),
              Row(
                children: [
                  Padding(
                    ...
                    child: Text(getMinute(currrentPosition)),
                  ),
                  Expanded(child: Container(decoration: BoxDecoration(borderRadius: BorderRadius.circular(10,),),
                   ...
                   child: Stack(children: [
                     Align(alignment: Alignment.center,child:widget.isReady?Container(margin: EdgeInsets.only(top: 0,left: 0,right: 0),child: showImage(),):Container(child:Center(child: CircularProgressIndicator())  ,) ,),


                  if(false)   Align(alignment: Alignment.center,child: Opacity(opacity: 0.3,
                                 ...
                                 if(snapshot.data == durationMillis)return   Container(
                                   ...
                                   child: ClipRRect(
                                     ...
                                     child: LinearProgressIndicator(
                                       value: 0,
                                       valueColor: AlwaysStoppedAnimation<Color>(Colors.blue.withOpacity(0.1)),
                  ...
                  Padding(
                    ...
                    child: Text(getMinute((durationMillis/1000).toInt())),
                  ),
                ],
              ),
         ...
         if(false) Align(alignment: Alignment.center,child: Container(decoration: BoxDecoration(borderRadius: BorderRadius.circular(10,),),
            margin: EdgeInsets.all(0),height: 125,
            child: Stack(children: [
...
class _SingleAudioGraphState extends State<SingleAudioGraph> {
  ...
  Widget build(BuildContext context) {
    ...
    return widget.isReady?Container(margin: EdgeInsets.only(top: 2,left: 2,right: 2,bottom: 2),child: Container(decoration: BoxDecoration(color: Colors.blue.withOpacity(0.1),borderRadius: BorderRadius.circular(5)
